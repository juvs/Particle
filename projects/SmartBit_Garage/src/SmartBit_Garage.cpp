...
void checkOpenDoorTime()
{
    if (openTime > 0)
    {
        ...
        if (minsOpenTimeout != 0 && diff >= DOOR_OPEN_TIMEOUT(minsOpenTimeout))
        { //If minsOpenTimeout is zero don't notify
            openTime = millis();
...
void updateRelayStatus()
{
    if (requestOpenClose == 1)
    {
        ...
        { //Solo cuando no estamos en modo lock
            relayOpenClose.pulse(250);
        }
    }

    if (lockMode == 1 && (doorStatus != -1 || doorStatus == 0))
    { //Lock only if is closed
        relayLock.on();
        relayPower.on();
    }
    else if (lockMode == 0)
...
void monitorMagneticSwitch()
{
    ...
    if (currentStatus == HIGH && (doorStatus == -1 || doorStatus == 0))
    { //El sensor esta en NO entonces LOW
        log("Puerta abierta");
        doorStatus = 1;
...
void checkButtonOpenCloseStatus()
{
    ...
    if (buttonOpenClose.clicks != 0)
        function = buttonOpenClose.clicks;
...
void changeOpenCloseState()
{
    ...
    { //Always update state because of openning state on ST
        notifyStatusToSTHub();
    }
}
...
void changeLockMode(int changeTo)
{
    ...
    { //If door is close you can locked
        lockMode = changeTo;
...
void notifyGarageOpenTimeoutToSTHub()
{
    log("Notificar garage X tiempo abierto");
    notifyJson["alarm"] = "open_garage_timeout";
    char jsonChar[512];
    serializeJson(notifyJson, jsonChar);
...
String getStatusJson()
{
    ...
    fromSTAction = 0;
    char jsonChar[512];
    serializeJson(statusJson, jsonChar);
...
void log(String msg)
{
    Serial.println(String("[SmartBit Garage] " + msg));
}